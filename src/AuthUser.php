    ...
    */
    ...
    protected function configure($requestForUserId = null, $options = [])
    {
        if (isEmpty($this->permissions) and is_array($this->permissions) === false) {
            throw new Exception('AuthUser - permissions empty. Please check your AuthUser configurations.');
        }

        if ($this->configEntity and !empty($this->configEntity) and $requestForUserId) {
            throw new Exception('AuthUser - use requestForUserId via checkEntity method.');
        }
        ...
        if (isEmpty($this->yesConfig) or isEmpty($this->configColRole) or isEmpty($this->configColUserId)) {
            throw new Exception('AuthUser - config item should contain col_role, col_user_id');
        }
        ...
        if ($requestForUserId and ($this->accessScope === 'user')) {
            ...
            if (!is_string($userModelString)) {
                throw new Exception('AuthUser - Please set key for user_model in config');
            }

            if (!class_exists($userModelString)) {
                throw new Exception('AuthUser - User model does not exist.');
            }
        ...
        if ($this->userIdentified) {
            ...
            if ($this->accessScope === 'role') {
                ...
                if ($remaingLevels = array_except($this->checkLevels, $this->roleLevels) and ($this->levelsModified === true)) {
                    ...
                    throw new Exception('Using AuthUser::checkUpto() with AuthUser::viaRole() is not permitted');
                }
    ...
    public function checkOnly($levels)
    {
        ...
        if (empty($this->checkLevels)) {
            throw new Exception('AuthUser::checkOnly() invalid array parameter'
    ...
    public function checkExcept($levels)
    {
        ...
        if (empty(array_only($this->checkLevels, $levels))) {
            throw new Exception('AuthUser::checkExcept() Invalid array parameter, '
                . implode(', ', array_keys($this->checkLevels)) . ' are accepted');
        }
    ...
    {
        if (!is_string($level)) {
            throw new Exception("AuthUser::checkUpto() - $level - argument should be string");
        }

        if (!array_key_exists($level, $this->checkLevels)) {
            throw new Exception(
                "Authuser::checkUpto() - Invalid key $level, Only "
                    . implode(', ', array_keys($this->checkLevels)) . ' are accepted'
            );
        }
    ...
    public function check($accessIdKey = null, $requestForUserId = null, array $options = [])
    {
        ...
        if (!$accessIdKey) {
            throw new Exception('AuthUser - AccessIdKey/RouteName is missing');
        }
        ...
        if (!is_string($accessIdKey)) {
            throw new Exception('AuthUser - Invalid AccessIdKey parameter for check');
        }
                ...
                foreach ($conditionItems as $conditionItem) {
                    ...
                    if ((isEmpty($conditionAccessIds) === false)) {
                        ...
                        if (($isMatchFound === true) and $uses and is_string($uses)) {
                            ...
                            $uses = explode('@', $uses);
                            if (count($uses) !== 2) {
                                throw new Exception('AuthUser invalid condition class configurations');
                            }

                            if (!class_exists($uses[0]) or !method_exists($uses[0], $uses[1])) {
                                throw new Exception('AuthUser invalid condition class or method configurations');
                            }
    ...
    protected function collectParentZones($accessZone, $allowDenyList, $intialAccessZone)
    {
        ...
        if ($parentZoneName == $accessZone) {
            throw new Exception("AuthUser - Zone's Self Parent Relation - $accessZone");
        }
        // if parent is there
        if ($parentZoneName) {
    ...
    protected function detailsFormat($isAccess, $accessIdKey, $options = [])
    {
        ...
        if ($isAccess instanceof AuthUserResult) {
    ...
    public function checkEntity($entityKey, $entityId, $requestForUserId = null)
    {
        ...
        if (!$entities or isEmpty($entities)) {
            throw new Exception('AuthUser - entities empty. Please check your AuthUser entities.');
        }
        ...
        if (!$this->configEntity or isEmpty($this->configEntity)) {
            throw new Exception('AuthUser - ' . $entityKey . ' entity not found. Please check your AuthUser entities.');
        }
        ...
        if (
            ...
            throw new Exception('AuthUser - entity config should contain model, permission_column and user_id_column');
        }

        if (!is_string($entityModelString)) {
            throw new Exception('AuthUser - Please set key for model in entity config');
        }

        if (!class_exists($entityModelString)) {
            throw new Exception('AuthUser - Entity model does not exist.');
        }
